...
A host restricting return expressions to write bindings must parse the expression as Python syntax and accept a single `ast.Name` only when its name is present in `ReturnExpression.validated_binding_name_to_value`. Declaring a write binding alone is insufficient: an executor may omit it from this candidate, leaving name resolution to an existing local or global value. Calls, attributes, indexing, and other expressions do not satisfy this name-only policy. Arbitrary `ast.Constant` values are a separate model-selected value path, not validated write bindings; any permitted constants need their own explicit host policy, such as a finite set of trusted sentinel values.

Name-only approval limits the expression's syntax, not the later work or final value. An awaitable binding is still awaited by an async Natural function, and core still validates the return against the function annotation. Validators may transform values or have effects even if the binding and return annotations are identical. A host requiring only a lookup must also reject awaitable values and account for return validation in its value policy. Validation occurs once per original binding/return candidate at each core boundary, not once globally across tools, host code, and both boundaries combined. An absent hook does not weaken core validation.

Final commit inspection remains necessary for hosts imposing a relationship between return values and committed writes. A binding-only Rewrite retains the previously resolved return, even if the approved expression was that binding's name. Replacements must preserve the host's intended relationship explicitly; core does not replay the expression or promise return-value identity with final assignments. A host policy limited to write bindings and capability requests must keep return denied until its own specification explicitly admits this route, including permitted constants, await behavior, validation, and rewrites.

### 10.4. Terminal delivery and host ledgers

`nh.scope(lifecycle=nh.lifecycle.StepLifecycle(on_step_finished=callback))` installs a synchronous callback accepting `StepFinished` and returning `None`. Omission or `UNSET` inherits; `None` clears. A new run resets lifecycle. Each execution captures the configuration on entry, before preparation. If that configuration has a handler, core attempts notification once while Python can unwind normally; a cleared or absent handler receives no notification. Definition-time parsing, decorator compilation, and failure to enter a required run are outside this boundary. Identity, step context when established, and the step span remain active during delivery and restore afterward, including when delivery fails.

The public `nh.get_lifecycle()` returns the currently scoped configuration object, or `None`, and requires an active run. Hosts can compare it by identity with their installed StepLifecycle at a host-controlled admission boundary. The getter observes configuration; it neither prevents `scope(lifecycle=None)` or nested-run resets nor guarantees later checks use the captured configuration of an already-started step. When a required lifecycle is absent, the host must record any mediation-bypass failure at a boundary that remains active, not rely on the removed callback. Core still wraps ordinary execution failures if there is no handler to translate them.
...
The callback may append the record to a host-owned authoritative ledger and raise a public host exception containing that exact stored event. For StepFailed, core explicitly chains the callback exception from `original_exception`; for StepRaised, from the prepared domain exception. If the callback rethrows the exact original exception object, core preserves its existing cause rather than chaining the exception to itself. Arbitrary callback exceptions are preserved and never reclassified or redelivered. `StepDeliveryError(execution_reference, cause)` is available for host adapters reporting storage failure; its `delivery_cause` does not imply a durable event exists. During ordinary execution failure, the outward chain prioritizes the execution exception and the storage cause remains available in `delivery_cause`.

Executor and commit-inspector exceptions produce StepFailed delivery even when the original exception is already a host-defined NaturalExecutionError. Core does not recognize host exception types or deduplicate host records. A handler that finds an already-recorded host exception must verify both matching execution identity and association with the exact stored event before skipping another append, then rethrow that exception to preserve its type. Returning normally instead causes core to raise ExecutionError chained from the host exception. A nested execution has a different step_execution_id and needs its own terminal event even when its failure carries a recorded child exception. Hosts should not suppress every exception of their public type indiscriminately.

On StepCompleted delivery failure, completed assignments remain visible and the callback's exception propagates. There is no original execution failure to fabricate as its cause, and the already-classified StepCompleted is not rewritten into StepFailed. Hosts may classify the overall operation as a commit/delivery failure and record their own anomaly; that classification is separate from core's execution result and does not authorize retrying the execution.

`StepInterrupted` holds the active `failure_stage` and original cancellation or process-control `BaseException`. Delivery is attempted synchronously without an additional await. If delivery also fails, the original interruption still propagates, explicitly chained from the delivery exception. Cancellation, SystemExit, KeyboardInterrupt, and GeneratorExit are not translated into ordinary host execution failures.
...
Host integrations can separate responsibilities: the executor boundary admits and accounts for execution, `inspect_step_commit` approves or rewrites the resolved candidate before assignment, and StepLifecycle records the terminal result and translates failures. Preparation can fail before executor admission; terminal consumers must accept records without a matching admission event. The shared `execution_reference.step_execution_id` correlates executor StepContext, inspectors, and terminal records without storing a live StepContext or pending-commit ContextVar. Terminal records make type-erasing validation workarounds unnecessary for catching binding, expression, and return-validation failures, but do not remove the host's other validation requirements. No external host code or host-specific serialization schema is provided by core. The deterministic ledger fixture in `tests/governance/test_step_ledger.py` demonstrates recording even when Python inside the Natural function catches the translated host exception.
